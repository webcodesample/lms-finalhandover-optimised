<?php 
	if(validateStaffAccessRight('calls',$con))
	{ 
?>
...
<div class="d-flex mb-3">
	<ul class="nav nav-underline fs-9">
		<li class="nav-item"><a href="callback_list.php" class="nav-link <?=$all_callback?>">All (<?=getRowCountWC('callback_list',$con)?>)</a></li>
...
while($callback = mysqli_fetch_assoc($callback_list))
{
	...
	$cb_lrn = getLatestUpdateRow('lrn',$callback['lrn'],'lrn_audit_history',$con);

	echo "<td>LA".$callback['lrn']."</td>
		<td>".$cb_lrn['name']."</td>
		<td>".$cb_lrn['mobile']."</td>";
