...
if(isset($_REQUEST['action_perform']))
{
	...
	if(getRowCountMultiCol(setWhereClause($where_con_array),'login_detail',$con))
	{
		...
		$loged_user_password = getFieldValueMultiCon('password',setWhereClause($where_con_array),'login_detail',$con);

		if(password_verify($_REQUEST['password'],$loged_user_password))
		{
			session_start();
			...
			else if(substr($loged_user_ref_id,0,2) == 'CR')
			{
				$_SESSION['login_type'] = 5;
				$_SESSION['user_table'] = "customers_list";
				$_SESSION['menu_id'] = 3;
			}
