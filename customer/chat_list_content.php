...
while($chat = mysqli_fetch_assoc($chat_list))
{
	if(substr($chat['chat_id'],0,2) == 'BS')
	{
		...
		$chat_user_type = "Chat Support";//getFieldValue('description','id',$role_type,'login_type_list',$con);
		$chat_name = "LMS V2.0";//getFieldValue('name','id',substr($chat['chat_id'],2),'staff_list',$con);
	}
	...
	else if(substr($chat['chat_id'],0,2) == 'CS')
	{
		$chat_user_type = "Chat Support";
		$chat_name = "LMS V2.0";
	}
	else
	{
	...
	if(preg_match($name_like,$chat_name))
	{
		echo "<div class='bg-primary-subtle border border-secondary-subtle m-1 rounded-5'>
			...
			<img src='../images/".$profile_pic."' class='rounded-circle bg-warning-subtle border border-1 border-primary px-1 pb-1 m-1' height='50px' width='50px'>
			<div class='p-2'>
