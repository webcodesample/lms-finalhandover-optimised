...
$where_con .= " AND cl.id != ".$_REQUEST['course_id'];

if(isset($_REQUEST['action_perform']))
{
	if(isset($_REQUEST['country']) && $_REQUEST['country'])
	{
		$country_arr = explode(' - ',$_REQUEST['country']);
		$where_con .= " AND ul.country_id = ".$country_arr[0];
	}

	if(isset($_REQUEST['university']) && $_REQUEST['university'])
	{
		$university_arr = explode(' - ',$_REQUEST['university']);
		$where_con .= " AND cl.university_id = '".$university_arr[0]."'";
	}
