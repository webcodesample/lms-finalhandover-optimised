...
while($sales_person = mysqli_fetch_assoc($sales_person_list))
{
	...
	echo "<tr>
		...
		<td><a class='text-decoration-none link-danger' href='manage_sales_person.php?sales_person_id=".$sales_person['id']."'>SP".$sales_person['id']."</a></td>
		<td>".$sales_person['email']."</td>
		<td>".$sales_person['mobile']."</td>
		<td>".$sales_person['whatsapp']."</td>
		<td nowrap>";
	if($sales_person['status'] == 1)
	echo "<a href='manage_sales_person_status.php?sales_person_id=".$sales_person['id']."&status=0' onclick='return makeConfirm(&quot;deactivate ".ucwords($sales_person['name'])."&quot;)'><span class='text-danger me-2' data-feather='power' style='height: 18px; width: 18px;'></span></a>";
	else
	echo "<a href='manage_sales_person_status.php?sales_person_id=".$sales_person['id']."&status=1' onclick='return makeConfirm(&quot;activate ".ucwords($sales_person['name'])."&quot;)'><span class='text-success me-2' data-feather='power' style='height: 18px; width: 18px;'></span></a>";

	echo "<a href='manage_sales_person.php?sales_person_id=".$sales_person['id']."' onclick='return makeConfirm(&quot;edit ".ucwords($sales_person['name'])."&quot;)'><img src='../portal-icons/edit.png' height='20px'></a>
		<a href='chat_history.php?chat_with=SP".$sales_person['id']."' target='_blank' class='mx-1' onclick='return makeConfirm(&quot;chat with ".ucwords($sales_person['name'])."&quot;)'><span class='text-success' data-feather='message-circle' style='height: 20px; width: 20px;'></span></a>
		</td>
		</tr>";
}
