...
{
	echo "<a href='urn_details.php?lead_id=".$_REQUEST['lead_id']."&tab=audit&audit_index=".($audit_history_offset-1)."' class='btn btn-subtle-primary btn-sm m-1'>New</a>";
}
else
{
...
{
	echo "<a href='urn_details.php?lead_id=".$_REQUEST['lead_id']."&tab=audit&audit_index=".($audit_history_offset+1)."' class='btn btn-subtle-primary btn-sm m-1'>Old</a>";
}
else
{
...
while($urn_audit_history = mysqli_fetch_assoc($result_audit_history))
{
	...
	/*$query_document_list = "SELECT file_name FROM urn_document_upload_history WHERE datentime <= '".$urn_audit_history['datentime']."'";

	while($document = mysqli_fetch_assoc(mysqli_query($con, $query_document_list)))
	{
		$documents_list .= "<li>".$document['file_name']."</li>";
	}

	$documents_list = "<ul>".$documents_list."</ul>";*/
	...
	echo "<center>
		...
		<tbody>
		<tr><td nowrap>Current Status </td><td>:</td><td>B".$current_status_batch_id."</td></tr>
	...
	if(str_contains($urn_audit_history['updated_columns'],'student_mobile'))
