...
<h5>Received Payment List On <?=$_REQUEST['date']?></h5>
...
while($rcvd_payment = mysqli_fetch_assoc($rcvd_payment_list))
{
	...
	$due_on_payment_date = $rcvd_payment['due_amount'];

	$lan_collection = mysqli_fetch_assoc(mysqli_query($con,"SELECT SUM(collected_amount) AS lan_collection FROM repayment_schedule_list WHERE lan = ".$rcvd_payment['lan']." AND due_date = ".$rcvd_payment['due_date']." AND collected_date = ".$payment_date))['lan_collection'];
		
	$due_balance_on_payment_date = $due_on_payment_date - $lan_collection;

	echo "<tr>
		<td class='ps-2'><input type='checkbox' name='dispatchLAN[]' class='form-check-input d-none me-1 dispatchLAN' value='".$rcvd_payment['lan']."' onClick='validateCheckCount()'>".$i."</td>
		...
		<td class='text-success fw-semibold'>".$lan_collection."</td>
