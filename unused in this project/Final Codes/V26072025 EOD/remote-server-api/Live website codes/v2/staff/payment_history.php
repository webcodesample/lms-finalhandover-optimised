...
while($payment = mysqli_fetch_assoc($payment_list))
{
	...
	if(substr($payment['comission_paid_to'],0,2) == "SA")
	{
		...
		$go_to_wallet_link = "manage_agent.php?agent_id=".substr($payment['comission_paid_to'],2)."&tab=ledger";
	}
	else if(substr($payment['comission_paid_to'],0,2) == "SP")
	{
		...
		$go_to_wallet_link = "manage_sales_person.php?sales_person_id=".substr($payment['comission_paid_to'],2)."&tab=ledger";
	}
