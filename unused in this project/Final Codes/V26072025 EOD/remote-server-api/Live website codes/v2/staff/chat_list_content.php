...
<div class="d-flex mb-3">
	<ul class="nav nav-underline">
		...
		<li class="nav-item"><a href="chat_list.php?tab=sa" class="nav-link <?=$agent_tab?>">Agent</a></li>
		<li class="nav-item"><a href="chat_list.php?tab=sp" class="nav-link <?=$sales_person_tab?>">Sales Pesron</a></li>
		<li class="nav-item"><a href="chat_list.php?tab=st" class="nav-link <?=$student_tab?>">Student</a></li>
	<ul>
</div>
...
while($chat = mysqli_fetch_assoc($chat_list))
{
	...
	else if(substr($chat['chat_id'],0,2) == 'SA')
	{
		$chat_user_type = "Agent";
		$chat_name = getFieldValue('name','id',substr($chat['chat_id'],2),'agent_list',$con);
	}
	else if(substr($chat['chat_id'],0,2) == 'SP')
	{
		$chat_user_type = "Sales Person";
		$chat_name = getFieldValue('name','id',substr($chat['chat_id'],2),'sales_person_list',$con);
	}
	else if(substr($chat['chat_id'],0,2) == 'ST')
	{
		$chat_user_type = "Student";
		$chat_name = getFieldValue('name','id',substr($chat['chat_id'],2),'students',$con);
	}
	else
	{
