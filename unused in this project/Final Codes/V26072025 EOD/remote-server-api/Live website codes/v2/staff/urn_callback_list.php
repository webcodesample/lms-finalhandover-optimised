...
<h6 class="mt-1">All Callbacks</h6>
<input type="checkbox" class="form-check-input m-1" checked onClick="$('#callbackList').toggleClass('d-none')">
</div>

<?php
$where_con_array_callback = [ 'lead_id' => $_REQUEST['lead_id'], ];

$order_clause = " ORDER BY callback_datentime DESC";

$callback_list = getRowsDataCon('callback_list',$where_con_array_callback,$con,$order_clause);

?>

<table id="callbackList" class="table table-sm table-bordered table-hover fs-9">
...
while($callback = mysqli_fetch_assoc($callback_list))
{
	...
	if($callback['updated_by'])
		$callback_status_updated_by = "<span class='text-danger'>".$callback['updated_by']." - </span>".strtoupper(getFieldValue('name','id',substr($callback['updated_by'],2),'staff_list',$con));
	else
	...
	{
		if($callback['callback_datentime'] < strtotime('now') && ($callback['callback_datentime']+600) > strtotime('now'))
		echo "<a href='add_callback_holder.php?callback_id=".$callback['id']."' class='mx-1'><span class='text-success' data-feather='edit' style='height: 15px; width: 15px;'></span></a>";
	}
