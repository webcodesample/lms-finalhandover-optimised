...
$where_con .= "AND ld.status_id = '".$_REQUEST['status']."'";
...
$query_view_lead = "SELECT *, ld.id AS lead_id, st.name AS student_name, cl.name AS course_name FROM leads_details AS ld JOIN course_list AS cl ON ld.course_id = cl.id JOIN students AS st ON ld.student_id = st.id ".$where_con." ORDER BY ld.datentime DESC";
