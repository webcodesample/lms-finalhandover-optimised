...
{
	$agent_tab = "active";
	$chat_list_query = "SELECT CONCAT('SA',id) AS chat_id FROM agent_list";
}
else if(isset($_REQUEST['tab']) && $_REQUEST['tab'] == 'sp')
{
	$sales_person_tab = "active";
	$chat_list_query = "SELECT CONCAT('SP',id) AS chat_id FROM sales_person_list";
}
else if(isset($_REQUEST['tab']) && $_REQUEST['tab'] == 'st')
{
	$student_tab = "active";
	$chat_list_query = "SELECT CONCAT('ST',id) AS chat_id FROM students";
}
else
{
