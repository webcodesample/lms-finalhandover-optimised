...
$prev_next_link_href = 'urn_details.php';
else
$prev_next_link_href = 'urn_history.php';
...
{
	echo "<a href='".$prev_next_link_href."?lead_id=".$_REQUEST['lead_id']."&tab=audit&audit_index=".($audit_history_offset-1)."' class='btn btn-subtle-primary btn-sm m-1'>New</a>";
}
else
{
...
{
	echo "<a href='".$prev_next_link_href."?lead_id=".$_REQUEST['lead_id']."&tab=audit&audit_index=".($audit_history_offset+1)."' class='btn btn-subtle-primary btn-sm m-1'>Old</a>";
}
else
{
...
while($urn_audit_history = mysqli_fetch_assoc($result_audit_history))
{
	...
	if($urn_audit_history['course_id'])
	{
	...
	{
		echo "<tbody id='course_selection_detail_".$i."' class='d-none'>
				<tr><td colspan=3>No Course selected</td></tr>
				</tbody>";
	}
