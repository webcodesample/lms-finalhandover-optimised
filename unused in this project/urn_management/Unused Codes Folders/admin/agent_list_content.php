...
while($agent = mysqli_fetch_assoc($agent_list))
{
	...
	echo "<tr>
		...
		<td nowrap>";
	if($agent['status'] == 1)
	echo "<a href='manage_agent_status.php?agent_id=".$agent['id']."&status=0'><span class='text-danger me-2' data-feather='power' style='height: 18px; width: 18px;'></span></a>";
	else
	echo "<a href='manage_agent_status.php?agent_id=".$agent['id']."&status=1'><span class='text-success me-2' data-feather='power' style='height: 18px; width: 18px;'></span></a>";

	echo "<a href='manage_agent.php?agent_id=".$agent['id']."'><img src='../portal-icons/edit.png' height='20px'></a>
		<a href='chat_history.php?chat_with=SA".$agent['id']."' target='_blank' class='mx-1'><span class='text-success' data-feather='message-circle' style='height: 20px; width: 20px;'></span></a>
		</td>
		</tr>";
}
