...
$callback_list_query = "SELECT * FROM callback_list AS CBL JOIN leads_details AS LD ON CBL.lead_id = LD.id WHERE LD.allocated_id = '".$_SESSION['agent_ref_id']."'";
...
$past_callback_list_query = "SELECT * FROM callback_list AS CBL JOIN leads_details AS LD ON CBL.lead_id = LD.id WHERE CBL.callback_datentime < ".strtotime('today')." AND LD.allocated_id = '".$_SESSION['agent_ref_id']."'";
...
$today_callback_list_query = "SELECT * FROM callback_list AS CBL JOIN leads_details AS LD ON CBL.lead_id = LD.id WHERE CBL.callback_datentime > ".strtotime('today')." AND CBL.callback_datentime < ".strtotime('tomorrow')." AND LD.allocated_id = '".$_SESSION['agent_ref_id']."'";
$today_callback_list = mysqli_query($con, $today_callback_list_query);
$today_callback_count = mysqli_num_rows($today_callback_list);

//today callback list & count for current agent
$future_callback_list_query = "SELECT * FROM callback_list AS CBL JOIN leads_details AS LD ON CBL.lead_id = LD.id WHERE CBL.callback_datentime > ".strtotime('tomorrow')." AND LD.allocated_id = '".$_SESSION['agent_ref_id']."'";
