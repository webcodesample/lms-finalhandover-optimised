...
if(substr($_SESSION['user_ref_id'],0,2) == 'SA')
$remark_suffix_part = "&agent_id=".substr($_SESSION['user_ref_id'],2);
else if(substr($_SESSION['user_ref_id'],0,2) == 'SP')
$remark_suffix_part = "&sales_person_id=".substr($_SESSION['user_ref_id'],2);
...
$wallet_balance = getFieldValue('balance','id',$wallet_id,'wallet_list',$con);
 ...
 while($wallet_transaction = mysqli_fetch_assoc($wallet_transactions))
 {
	 ...
	 if($wallet_transaction['transaction_type'] == 111)
		...
		$remark_suffix = " <a href='../transaction_proof_management/transaction_detail.php?transaction_id=".$wallet_transaction['transaction_id'].$remark_suffix_part."' class='link-success text-decoration-none'>".$wallet_transaction['transaction_id']."</a>";
