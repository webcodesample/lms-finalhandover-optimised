...
function getChatUserLoginStatus()
{
    $.ajax({
        url: "../common/getChatUserLoginStatus_ajax.php",
        type: "post",
        data: {
            logged_user: "<?=$_SESSION['user_ref_id']?>",
...
function createChatRecord()
{
    if($("#chat_msg").val() || $("#chatPhotos").val())
    {
        ...
        if($("#chatPhotos").val() && ($('#chatPhotos').prop('files')[0]['size'] > 222429))
        {
            alert("File sholud be upto 200KB");
            getChatMessages();
        }
        else
        {
