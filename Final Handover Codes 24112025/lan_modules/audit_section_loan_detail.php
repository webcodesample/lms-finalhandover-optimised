<?php
if($lan_audit_history['product_id'] > 0)
$product = getFieldValue('description','id',$lan_audit_history['product_id'],'product_list',$con);
else
$product = 'Not Selected';

if($lan_audit_history['introducer_id'] > 0)
$introducer = getFieldValue('name','id',$lan_audit_history['introducer_id'],'introducer_list',$con);
else
...
echo "<thead>
	...
	<tr>
		...
		<td>".validateColumnUpdation('required_loan_amount',$lan_audit_history['required_loan_amount'],$updated_column_array)."</td>
	</tr>
	<tr>
...
if($lan_audit_history['approved_loan_amount'] > 0)
{
	$payment_frequency = $lan_audit_history['payment_frequency'];
	$roi = $lan_audit_history['roi'].'% '.$payment_frequency;
	$loan_tenure = $lan_audit_history['loan_tenure'].' '.str_replace('ly','s',$payment_frequency);

	echo "<tr>
			<td nowrap>Approved Amount </td><td>:</td>
			<td>".validateColumnUpdation('approved_loan_amount',$lan_audit_history['approved_loan_amount'],$updated_column_array)."</td>
		</tr>
		<tr>
...
echo "<tr>
		<td nowrap>Credit Limit</td><td>:</td>
		<td>".validateColumnUpdation('credit_limit',$lan_audit_history['credit_limit'],$updated_column_array)."</td>
	</tr>";
